...
namespace SDF::Common::Data::Adapters {
  ...
  public:
    ...
    bool
    exist(FilesystemKey key);

    void
    create(FilesystemKey key);

    void
    writeData(FilesystemKey key, const std::vector<unsigned char> &data);

    void
    readData(FilesystemKey key, std::vector<unsigned char> &destination);

    void
    erase(FilesystemKey key);
  };
}
