...
namespace SDF::ModelLayer::DomainObjects::Image::Gil {
  ...
  template<class GilImageType, class GilRegionType, class GilPixelType>
  std::size_t
  Layer<GilImageType, GilRegionType, GilPixelType>::getWidthPx() const {
    return m_layerPixelData.width();
  }

  template<class GilImageType, class GilRegionType, class GilPixelType>
  std::size_t
  Layer<GilImageType, GilRegionType, GilPixelType>::getHeightPx() const {
