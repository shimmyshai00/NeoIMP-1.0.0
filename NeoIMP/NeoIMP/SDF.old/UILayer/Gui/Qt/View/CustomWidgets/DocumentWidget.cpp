...
namespace SDF::UILayer::Gui::Qt::View::CustomWidgets {
  ...
  float DocumentWidget::centerX() const {
    return m_documentEditorWidget->centerX();
  }

  float DocumentWidget::centerY() const {
    return m_documentEditorWidget->centerY();
  }
  ...
  void DocumentWidget::setCenterX(float centerX) {
    m_documentEditorWidget->setCenterX(centerX);
  }

  void DocumentWidget::setCenterY(float centerY) {
    m_documentEditorWidget->setCenterY(centerY);
  }

  void DocumentWidget::setMagnification(float magnification) {
    QPointF curCenter(centerX(), centerY());

    m_documentEditorWidget->setZoom(magnification);
    m_documentEditorWidget->setCenterX(curCenter.x());
    m_documentEditorWidget->setCenterY(curCenter.y());
  }
  ...
  DocumentWidget::centerViewAt(QPointF centerPoint, bool noScrollUpdate) {
    ...
    m_documentEditorWidget->setCenterX(centerPoint.x());
    m_documentEditorWidget->setCenterY(centerPoint.y());
  }
