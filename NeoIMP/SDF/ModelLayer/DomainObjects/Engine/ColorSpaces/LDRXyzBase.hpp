#ifndef SDF_MODELLAYER_DOMAINOBJECTS_ENGINE_COLORSPACES_LDRXYZBASE_HPP
#define SDF_MODELLAYER_DOMAINOBJECTS_ENGINE_COLORSPACES_LDRXYZBASE_HPP
 ...
 *
 * FILE:    LDRXyzBase.hpp
 * PURPOSE: Defines the LDRXyzBase template.
 */
...
namespace SDF::ModelLayer::DomainObjects::Engine::ColorSpaces {
  // Class:      LDRXyzBase
  // Purpose:    Defines a color-space base for low-dynamic range (i.e. fixed point/discrete color
  //             values, aka. most common image formats) spaces mapping to CIE 1931 XYZ as their
  //             fundamental.
  // Parameters: PixelDataT - The type of pixel data accepted.
  template<class PixelDataT>
  class LDRXyzBase : public IColorSpace<PixelDataT> {
  public:
    virtual ~LDRXyzBase() = default;

    // Function:   LDRXyzBase
    // Purpose:    Builds the color space on the given color model.
    // Parameters: colorModel - The color model going in.
    // Returns:    None.
    LDRXyzBase(const IColorModel<PixelDataT> *colorModel);
